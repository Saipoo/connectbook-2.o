...
export const seedInternshipsAndHackathons = async () => {
  try {
    ...
    const expectedInternships = dummyInternships.length + realInternships.length;
    const expectedHackathons = dummyHackathons.length + realHackathons.length;
    
    // Always clear and re-seed (as per user request)
    console.log('🗑️  Clearing existing data for fresh seed...');
    await Internship.deleteMany({});
    await HackathonChallenge.deleteMany({});
    
    // Drop any old indexes that might cause conflicts
    try {
      await Internship.collection.dropIndexes();
      console.log('✅ Dropped old internship indexes');
    } catch (error) {
      console.log('ℹ️  No old indexes to drop for internships');
    }
    
    try {
      await HackathonChallenge.collection.dropIndexes();
      console.log('✅ Dropped old hackathon indexes');
    } catch (error) {
      console.log('ℹ️  No old indexes to drop for hackathons');
    }
    
    // Recreate indexes based on current schema
    await Internship.createIndexes();
    await HackathonChallenge.createIndexes();
    console.log('✅ Recreated schema indexes');

    console.log('🌱 Starting fresh seed process for internships and hackathons...');

    // Seed internships (both dummy and real)
    ...
    console.log(`✅ Successfully seeded ${allInternships.length} internships (${dummyInternships.length} simulation + ${realInternships.length} real)`);

    // Seed hackathons (both dummy and real)
    const allHackathons = [...dummyHackathons, ...realHackathons];
    console.log(`🏆 Inserting ${allHackathons.length} hackathons...`);
    await HackathonChallenge.insertMany(allHackathons);
    console.log(`✅ Successfully seeded ${allHackathons.length} hackathons (${dummyHackathons.length} simulation + ${realHackathons.length} real)`);

    console.log('🎉 Internships and hackathons seed completed!');
  } catch (error) {
    console.error('❌ Error seeding internships/hackathons:', error);
